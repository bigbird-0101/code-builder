...
import com.fpp.code.domain.CoreConfig;
import com.fpp.code.domain.DataSourceConfig;
import com.fpp.code.filebuilder.DefaultFileBuilder;
import com.fpp.code.filebuilder.FileAppendSuffixCodeBuilderStrategy;
import com.fpp.code.filebuilder.FileBuilder;
import com.fpp.code.filebuilder.FileCodeBuilderStrategy;
import com.fpp.code.filebuilder.definedfunction.DefaultDefinedFunctionResolver;
import com.fpp.code.template.*;
...
public class MainJFramePage extends JFrame {
    private JFramePageEnvironment jFramePageEnvironment;
    /**
     * 选中的组合模板
     */
    ...
    private JComponent firstPage;
    private JComponent configPage;
    private JComponent aboutPage;
    public MainJFramePage() throws HeadlessException {
    }
    public MainJFramePage(JFramePageEnvironment environment) {
        this.jFramePageEnvironment = environment;
    }
    ...
    private boolean hasHandleFunctionTemplate() throws Exception {
        return this.targetMultipleTemplate.getMultipleTemplate(jFramePageEnvironment.getProjectFileConfig()).stream().anyMatch(s -> s instanceof HandleFunctionTemplate);
    }
    ...
    public ProjectTemplateInfoConfig getProjectTemplateInfoConfig() {
        ...
        return new ProjectTemplateInfoConfig(handleFunctionConfig, noHandleFunctionConfig, jFramePageEnvironment.getProperty("project-complete-url"), jFramePageEnvironment.getProperty("project-target-packageurl"), definedFunctionDomainList);
    }
    ...
    public DataSourceConfig getDataSourceConfig() {
        String url = jFramePageEnvironment.getProperty("url");
        String quDongName = url.indexOf("mysql") > 0 ? "com.mysql.jdbc.Driver" : url.indexOf("oracle") > 0 ? "" : "";
        return new DataSourceConfig(quDongName, jFramePageEnvironment.getProperty("username"), url, jFramePageEnvironment.getProperty("password"));
    }
    /**
     * 重新设置配置
     */
    private void reloadConfig(Map<String, String> newConfig) {
        ...
        try {
            if (!properties.isEmpty()) {
                jFramePageEnvironment.getDataSourceFileConfig().coverProperty(properties);
            }
        } catch (IOException e) {
            e.printStackTrace();
        }
        ...
        try {
            if (!propertiesProject.isEmpty()) {
                jFramePageEnvironment.getProjectFileConfig().coverProperty(propertiesProject);
            }
        } catch (IOException e) {
            e.printStackTrace();
        }
    ...
    public JComponent getTemplateInfoPage() throws Exception {
        ...
        //生成模板页面
        JPanel jPanelPage = createTemplatePage(jFramePageEnvironment.getMultipleTemplates().get(0));
    ...
    public JPanel getSelectTemplatePage() {
        ...
        comboBox.addItemListener(e -> {
            ...
            if (e.getStateChange() == ItemEvent.SELECTED) {
                this.targetMultipleTemplate = jFramePageEnvironment.getMultipleTemplates().get(comboBox.getSelectedIndex());
        ...
        if (null == this.targetMultipleTemplate) {
            this.targetMultipleTemplate = jFramePageEnvironment.getMultipleTemplates().get(0);
        }
        ...
        reloadBotton.addActionListener(e -> {
            try {
                jFramePageEnvironment.refresh();
                MultipleTemplate multipleTemplate = jFramePageEnvironment.getMultipleTemplates().get(0);
                multipleTemplate.getMultipleTemplate(null).forEach(template -> {
                    if (template instanceof AbstractTemplate) {
                        AbstractTemplate abstractTemplate = (AbstractTemplate) template;
                        try {
                            abstractTemplate.refresh();
    ...
    public JPanel createTemplatePage(MultipleTemplate multipleTemplate) throws Exception {
        ...
        List<Template> templateList = multipleTemplate.getMultipleTemplate(jFramePageEnvironment.getProjectFileConfig());
    ...
    private boolean haveNoHandleTemplate(String templateName) {
        String property = jFramePageEnvironment.getProperty("nohandle-template-build");
    ...
    private void loadHandleFunctionPage(JPanel jPanelPage, List<Template> templateList, Integer[] totalWidthTotalHeight) throws TemplateResolveException {
        ...
        for (HandleFunctionTemplate handleFunctionTemplate : handleFunctionTemplateList) {
            ...
            JCheckBoxGroup jCheckBoxGroup = new JCheckBoxGroup(functionNameS, templateName, jPanel2, jFramePageEnvironment.getProjectFileConfig());
    ...
    public boolean checkSelectAllFunctionWhenTemplate(HandleFunctionTemplate template) throws TemplateResolveException {
        String property = jFramePageEnvironment.getProperty("handle-template-build");
        if (Utils.isEmpty(property)) {
            return true;
        }
    ...
    public JPanel getConfigPage() {
        ...
        textArea.setText(jFramePageEnvironment.getProperty("url"));
        // 设置自动换行
        ...
        textField_1.setText(jFramePageEnvironment.getProperty("username"));
        ...
        textField_2.setText(jFramePageEnvironment.getProperty("password"));
        ...
        textFiled_5.setText(jFramePageEnvironment.getProperty("project-complete-url"));
        ...
        textField_3.setText(jFramePageEnvironment.getProperty("project-target-packageurl"));
        ...
        this.configPage = config;
        return config;
    }

    public JPanel getAboutPage() {
        JPanel aboutPage = new JPanel();
        aboutPage.setLayout(null);
        JLabel label = new JLabel("github 持续更新中...,期待您的Star");
        label.setBounds(156, 81, 400, 15);
        aboutPage.add(label);
        JLabel label2 = new LinkLabel("地址:https://github.com/bigbird-0101/spring-code", "https://github.com/bigbird-0101/spring-code");
        label2.setBounds(156, 121, 400, 15);
        aboutPage.add(label2);
    ...
    private void initJMenuBar() {
        ...
        map.put("首页", new MenuBarPage.JMenuPageInfo(new MenuBarPage.MenuListenerImpl(() -> {
            showSpecifiedPanel(null != this.firstPage ? this.firstPage : getFirstPage());
        })));
        map.put("数据源和项目配置", new MenuBarPage.JMenuPageInfo(new MenuBarPage.MenuListenerImpl(() -> {
            showSpecifiedPanel(null != this.configPage ? this.configPage : getConfigPage());
        })));
        map.put("关于", new MenuBarPage.JMenuPageInfo(new MenuBarPage.MenuListenerImpl(() -> {
            showSpecifiedPanel(null != this.aboutPage ? this.aboutPage : getAboutPage());
        })));
    ...
    public Template getTemplateByName(String templateName) throws CodeConfigException, IOException {
        for (MultipleTemplate multipleTemplate : jFramePageEnvironment.getMultipleTemplates()) {
            for (Template template : multipleTemplate.getMultipleTemplate(jFramePageEnvironment.getProjectFileConfig())) {
    ...
    public Template getTemplateFromTargetByName(String templateName) throws IOException, CodeConfigException {
        return targetMultipleTemplate.getMultipleTemplate(jFramePageEnvironment.getProjectFileConfig()).stream().filter(item -> item.getTemplateName().equals(templateName)).findFirst().get();
    }
    ...
    public String[] getTeplateNameAllMultipleTemplate() {
        return jFramePageEnvironment.getMultipleTemplates().stream().map(MultipleTemplate::getTemplateName).collect(Collectors.toList()).toArray(new String[jFramePageEnvironment.getMultipleTemplates().size()]);
    }

    public JFramePageEnvironment getjFramePageEnvironment() {
        return jFramePageEnvironment;
    }

    public void setjFramePageEnvironment(JFramePageEnvironment jFramePageEnvironment) {
        this.jFramePageEnvironment = jFramePageEnvironment;
    }
    /**
     * 当前主窗口的监听
     */
