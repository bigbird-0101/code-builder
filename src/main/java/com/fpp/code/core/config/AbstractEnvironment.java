...
public abstract class AbstractEnvironment implements Environment {
    ...
    public void parse() throws CodeConfigException {
        try {
            loadCoreConfig();
            getPropertySources().addPropertySource(new GeneratePropertySource<>(DEFAULT_CORE_TEMPLATE_PATH, getTemplateConfigPath()));
            getPropertySources().addPropertySource(new GeneratePropertySource<>(DEFAULT_CORE_TEMPLATE_FILES_PATH, getTemplatesPath()));
            loadTemplatesPath();
    ...
    public void loadCoreConfig(String fileName) throws IOException {
        ...
        pss.stringPropertyNames().forEach((k) -> {
            ...
            GeneratePropertySource generatePropertySource = new GeneratePropertySource(k, pss.getProperty(k));
            getPropertySources().addPropertySource(generatePropertySource);
        });
    }
