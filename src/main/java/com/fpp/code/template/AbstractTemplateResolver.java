...
public abstract class AbstractTemplateResolver implements TemplateResolver {
    ...
    protected static final String CONFIG_TEMPLATE_RESOLVERS="com.fpp.code.template-lang-resolver";
    ...
    public AbstractTemplateResolver(ProjectFileConfig projectFileConfig) throws CodeConfigException {
        init();
        this.projectFileConfig=projectFileConfig;
        initTemplateLangResolverListWhenConfig(projectFileConfig);
    }
    ...
    private void  initTemplateLangResolverListWhenConfig(ProjectFileConfig projectFileConfig) throws CodeConfigException {
        List<TemplateLangResolver> templateLangResolverListTemp;
        if(null!=projectFileConfig) {
            String templateLangResolverListClassNameStr = projectFileConfig.getProperty(CONFIG_TEMPLATE_RESOLVERS);
            if (Utils.isNotEmpty(templateLangResolverListClassNameStr)) {
                ...
                for (String classNameLangResolver : classNameLangResolverArray) {
                    try {
                        ...
                        e.printStackTrace();
                        throw new CodeConfigException("配置文件:"+ProjectFileConfig.getConfigPrefix()+CONFIG_TEMPLATE_RESOLVERS+"配置项,类"+classNameLangResolver+"未找到");
