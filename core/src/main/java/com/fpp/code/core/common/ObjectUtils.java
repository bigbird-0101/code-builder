...
public abstract class ObjectUtils {
    ...
    public static void copyProperties(Object source, Object target) {
        List<PropertyDescriptor> propertyDescriptors = getPropertyDescriptors(target.getClass());
        for (PropertyDescriptor propertyTargetDescriptor : propertyDescriptors) {
            if (null == propertyTargetDescriptor.getWriteMethod()) {
                continue;
            }
            PropertyDescriptor propertySourceDescriptor = getPropertyDescriptor(source.getClass(), propertyTargetDescriptor.getName());
            if (null == propertySourceDescriptor || null == propertySourceDescriptor.getReadMethod()) {
                continue;
            }
            try {
                Method readMethod = propertySourceDescriptor.getReadMethod();
                Method writeMethod = propertyTargetDescriptor.getWriteMethod();
                if (readMethod != null &&
    ...
    private static Object getFieldValue(Object source, String name) {
        return Stream.of(source.getClass().getSuperclass().getDeclaredFields())
                .filter(s -> s.getName().equals(name)).findFirst()
                .map(s -> ReflectionUtils.getField(s, source))
                .orElse(
                        Stream.of(source.getClass().getSuperclass().getFields())
    ...
    public static List<PropertyDescriptor> getPropertyDescriptors(Class<?> clazz) {
        try {
            return Stream.of(Introspector.getBeanInfo(clazz).getPropertyDescriptors()).collect(Collectors.toList());
    ...
    public static PropertyDescriptor getPropertyDescriptor(Class<?> clazz, String fieldName) {
        try {
            return Stream.of(Introspector.getBeanInfo(clazz).getPropertyDescriptors())
                    .filter(s -> s.getName().equals(fieldName)).findFirst().orElse(null);
        } catch (IntrospectionException e) {
            throw new IllegalArgumentException(e);
        }
    }

    public static List<PropertyDescriptor> getPropertyDescriptors2(Class<?> clazz) {
        List<PropertyDescriptor> propertyDescriptorList = new ArrayList<>();
        List<Field> fieldSourceList = Arrays.asList(clazz.getFields());
        List<Field> fieldDeclaredSourceList = Arrays.asList(clazz.getDeclaredFields());
        if (!CollectionUtils.isEmpty(fieldSourceList)) {
            for (Field field : fieldSourceList) {
                try {
                    propertyDescriptorList.add(new PropertyDescriptor(field.getName(), clazz));
                } catch (IntrospectionException e) {
                    throw new IllegalArgumentException(e);
                }
            }
        }
        if (!CollectionUtils.isEmpty(fieldDeclaredSourceList)) {
            for (Field field : fieldDeclaredSourceList) {
                try {
                    propertyDescriptorList.add(new PropertyDescriptor(field.getName(), clazz));
                } catch (IntrospectionException e) {
                    throw new IllegalArgumentException(e);
